...
public class BezieMissile : MonoBehaviour
{
    Vector3[] point = new Vector3[4];       //4개의 베지어 컨트롤 포인트를 저장할 배열
    bool hit = false;                       //미사일이 목표에 도달했는지 여부를 나타내는 변수

    [SerializeField] [Range(0f, 1f)] float t = 0;    //베지어 곡선에서의 시간 변수
    [SerializeField] public float spd = 5;          //미사일의 이동속도
    [SerializeField] public float posA = 0.55f;     //ai미사일 궤적을 제어하는 파라미터 A
    [SerializeField] public float posB = 0.45f;     //ai미사일 궤적을 제어하는 파라미터 B
    ...
    public void Start()
    {
        ...
        point[2] = PointSetting(master.transform.position);     //랜덤 컨트롤
    ...
    Vector3 PointSetting(Vector3 origin)
    {
        ...
        y = posB * Mathf.Cos(Random.Range(0, 360) * Mathf.Deg2Rad) + origin.y;
        z = posA * Mathf.Cos(Random.Range(0, 360) * Mathf.Deg2Rad) + origin.z;
    ...
    {
        //4개의 컨트롤 포인트를 사용해서 베지어 곡선에서의 값을 계산
